...
def policy_iteration(env, gamma, max_iterations, logger):
    ...
    gamma: float
        the reward discount factor
    max_iterations: integer
        the maximum number of global policy iterations that should be performed;
        the algorithm should terminate when max_iterations is exceeded. 
        Each iteration involves a policy evaluation step and a policy improvement step. 
...
def q_learning(env, gamma, max_iterations, logger):
    ...
    IMPORTANT: In RL, you do not have access to the transition model 
